...
timeLet='timeLetMemo.log'
memoryLet='memoryLetMemo.log'
...
do 
    { /usr/bin/time -p python3 letMemo.py $input_size > /dev/null; } 2>&1 | grep real | grep -Eo '[0-9]+([.][0-9]+)?' | head -c -1 >> $timeLet
...
for input_size in $sequenceLet
do 
    { /usr/bin/time -v python3 letMemo.py $input_size > /dev/null; } 2>&1 | grep "Maximum resident" | sed "s/[^0-9]\+\([0-9]\+\).*/\1/"  | head -c -1 >> $memoryLet
